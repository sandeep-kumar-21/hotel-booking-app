...
function RoomDetails() {
  ...
  const checkAvailability = async () => {
    try{
        ...
        // console.log("checkAvailability catch error : " + error.message)
    }
  }
    ...
    try{
        ...
        if(!isAvailable){
            ...
            if(data.success){
                ...
                toast.error(data.message)
                console.log('onSubmitHandler in RoomDetails else error :' +data.message)
            }
        }
    }catch(error){
        toast.error(error.message)
        console.log('onSubmitHandler in RoomDetails catch error :' + error.message);
    }
  }
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32' >
        ...
        <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
            <h1 className='text-3xl md:text-4xl font-payfair' >{room.hotel.name} <span className='font-inter text-sm' >{room.roomType}</span> </h1>
        ...
        <div className='max-w-3xl border-y border-gray-300 my-15 py-10 text-gray-500' >
            <p>Guests will be allocated on the ground floor according to availability. You get a comfortable Two bedroom aprtment has a trye city feeling. The price quoted is for two guest, at the guest slot pleasee mark the number of guests to get exact price for groups . the Guests will be allocated gound floor according to availability. You get the comfortble two bedroom apartment that has a true city feeling. </p>
        </div>
        ...
        <div className='flex flex-col itmes-start gap-4' >
            ...
            <button className='px-6 py-2.5 mt-4 w-40 rounded text-white bg-primary hover:bg-primary-dull transition-all cursor-pointer'>Contact Now</button>
        </div>
    </div>
  )
