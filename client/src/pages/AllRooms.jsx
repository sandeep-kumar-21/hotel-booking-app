...
const CheackBox = ({label,selected = false , onChange = () => {}}) => {
    return (
...
function AllRooms() {
  ...
  const sortRooms = (a, b)  => {
    ...
    if(selectedSort === 'Newest First'){
        return new Date(b.createdAt) - new Date(a.checkedAt);
    }
    return 0;
  }
  ...
  //Filter and sort rooms based on the selected filter and sort option
            ...
            <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`} >
                <div className='px-5 pt-5' >
                    ...
                    {roomTypes.map((room,index) => (
                        <CheackBox key={index} label={room} selected={selectedFilters.roomType.includes(room)}
                ...
                <div className='px-5 pt-5' >
                    ...
                    {priceRanges.map((range,index) => (
                        <CheackBox key={index} label={`${currency}${range}`} selected={selectedFilters.priceRanges.includes(range)}
